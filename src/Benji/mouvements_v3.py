...
try:
    ...
    sprite_idle  = pygame.Surface((80, 120)); sprite_idle.fill((100, 180, 255))
...
# ---------------------------------------------------------------------------
salle_actuelle      = 1
joueur_x            = SALLES[1]['depart'][0]
joueur_y            = SALLES[1]['depart'][1]
x_direction         = 0
y_direction         = 0
sprite_courant      = sprite_idle
mode                = "exploration"   # "exploration" | "enigme" | "porte" | "victoire"
enigme_active       = None
saisie              = ""
message_retour      = ""
message_timer       = 0
...
def draw_text_wrapped(surface, text, x, y, max_width, color=(255,255,255), f=None):
    if f is None:
        f = font_small
    for line in text.split('\n'):
        ...
        for word in words:
            ...
            if f.size(test)[0] <= max_width:
                ...
                if current_line:
                    surface.blit(f.render(current_line, True, color), (x, y))
                    y += f.get_linesize()
                current_line = word
        if current_line:
            surface.blit(f.render(current_line, True, color), (x, y))
...
def draw_porte_panel():
    ...
    titre = font.render("— PORTE DE SORTIE —", True, (220, 190, 100))
    ...
    if message_porte_timer > 0:
        ...
        col = (80, 255, 80) if ok else (255, 80, 80)
    ...
    pygame.draw.rect(screen, (180, 150, 80), saisie_rect, 2, border_radius=8)
    saisie_txt = font.render(saisie_porte + "|", True, (255, 255, 255))
...
def draw_hud():
    txt = font_small.render(
        f"Salle {salle_actuelle} / {NB_SALLES}   |   Fleches : bouger   E : interagir   M/L : musique",
        True, (220, 220, 180))
...
def draw_objects():
    ...
    for enig in salle['enigmes']:
        ...
        pygame.draw.rect(screen, color,
                         [enig['x'], enig['y'], enig['largeur'], enig['hauteur']], 3, 6)
...
def draw_door():
    ...
    pygame.draw.rect(screen, (255, 255, 255), [px, py, 80, 130], 2, 8)
    lbl = font_small.render("SORTIE" if all_solved else "Verrouillee", True, (255, 255, 255))
...
def draw_interaction_hints():
    ...
    for enig in salle['enigmes']:
        if not enig['resolu']:
            zone = pygame.Rect(enig['x'] - 60, enig['y'] - 60,
                               enig['largeur'] + 120, enig['hauteur'] + 120)
            if joueur_rect.colliderect(zone):
                hint = font_small.render("E — Examiner", True, (255, 255, 0))
    ...
    if joueur_rect.colliderect(porte_zone):
        all_solved = all(e['resolu'] for e in salle['enigmes'])
        if all_solved:
            hint = font_small.render("E — Entrer le code", True, (100, 255, 100))
        else:
            hint = font_small.render("Resous les enigmes d abord !", True, (255, 80, 80))
...
def draw_enigme_panel():
    ...
    titre = font.render(f"— {enigme_active['label']} —", True, (220, 190, 100))
    screen.blit(titre, (panel.x + panel.w//2 - titre.get_width()//2, panel.y + 18))

    draw_text_wrapped(screen, enigme_active['indice'],
    ...
    pygame.draw.rect(screen, (50, 45, 65), saisie_rect, border_radius=8)
    pygame.draw.rect(screen, (180, 150, 80), saisie_rect, 2, border_radius=8)
    saisie_txt = font.render(saisie + "|", True, (255, 255, 255))
    screen.blit(saisie_txt, (saisie_rect.x + 10, saisie_rect.y + 8))
...
def draw_victoire():
    ...
    sub = font.render("Tu as resolu toutes les enigmes et quitte le Louvre !", True, (220, 220, 220))
    screen.blit(sub, (WIDTH//2 - sub.get_width()//2, HEIGHT//2))
    restart = font_small.render("Appuie sur R pour recommencer", True, (160, 160, 160))
...
def passer_salle_suivante():
    global salle_actuelle, joueur_x, joueur_y, mode
...
def valider_reponse():
    global saisie, message_retour, message_timer, mode, enigme_active
    ...
    if saisie.strip().lower() == enigme_active['reponse'].lower():
        ...
        message_retour = "Mauvais code... Reessaie."
        message_timer  = 90
    saisie = ""


def valider_code_porte():
    global saisie_porte, message_porte, message_porte_timer
    salle = SALLES[salle_actuelle]
...
def reset_game():
    global salle_actuelle, joueur_x, joueur_y, mode, enigme_active
    global saisie, message_retour, message_timer, x_direction, y_direction, sprite_courant
    global saisie_porte, message_porte, message_porte_timer
